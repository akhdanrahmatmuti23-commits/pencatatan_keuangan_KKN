...
$transactions = [];
$stmt = mysqli_prepare($conn, 'SELECT date, description, category, type, amount FROM transactions WHERE user_id = ? ORDER BY date DESC, id DESC LIMIT 6');
mysqli_stmt_bind_param($stmt, 'i', $userId);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
...
<head>
    ...
    <title>Dashboard KKN Keuangan</title>
    ...
    <style>
        ...
        /* Kustomisasi scrollbar halus ala iOS */
        ::-webkit-scrollbar { width: 5px; height: 5px; }
        ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background: rgba(140, 67, 53, 0.15); border-radius: 10px; }
    </style>
</head>
<body class="min-h-screen text-slate-900 flex flex-col md:flex-row pb-24 md:pb-0 antialiased">
    ...
    <aside class="hidden md:flex fixed top-4 left-4 bottom-4 w-64 bg-white/40 backdrop-blur-2xl border border-white/40 p-6 flex-col justify-between rounded-[32px] shadow-[0_20px_50px_rgba(140,67,53,0.08)] z-50">
        <div>
            ...
            <nav class="space-y-1">
                <a href="user.php" class="flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-2xl bg-[#8c4335]/10 text-[#8c4335] border border-[#8c4335]/5 shadow-[0_4px_12px_rgba(140,67,53,0.04)] transition-all">
    ...
    <nav class="md:hidden fixed bottom-4 left-4 right-4 z-50 bg-white/60 backdrop-blur-2xl border border-white/40 shadow-[0_15px_40px_rgba(140,67,53,0.12)] px-2 py-2 flex justify-around items-center rounded-[24px]">
        <a href="user.php" class="flex flex-col items-center gap-1 py-1.5 px-4 rounded-xl bg-[#8c4335]/10 text-[#8c4335] font-semibold text-[10px] tracking-tight transition-all">
        ...
        <a href="pemasukan.php" class="flex flex-col items-center gap-1 py-1.5 px-4 rounded-xl text-slate-500 font-medium text-[10px] tracking-tight transition-all active:scale-95">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 18zm1-11a1 1 0 10-2 0v3.586L7.707 9.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 10.586V7z" clip-rule="evenodd" /></svg>
            <span>Pemasukan</span>
        </a>
        <a href="pengeluaran.php" class="flex flex-col items-center gap-1 py-1.5 px-4 rounded-xl text-slate-500 font-medium text-[10px] tracking-tight transition-all active:scale-95">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 18zm1-11a1 1 0 10-2 0v3.586L7.707 9.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 10.586V7z" clip-rule="evenodd" transform="rotate(180 10 10)" /></svg>
            <span>Pengeluaran</span>
        </a>
        <a href="profil.php" class="flex flex-col items-center gap-1 py-1.5 px-4 rounded-xl text-slate-500 font-medium text-[10px] tracking-tight transition-all active:scale-95">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" /></svg>
            <span>Profil</span>
        </a>
    </nav>
    ...
    <div class="flex-1 flex flex-col min-w-0 md:pl-4">
        ...
        <header class="bg-white/20 backdrop-blur-xl border-b border-white/30 px-6 py-4 md:px-8 flex items-center justify-between sticky top-0 z-30">
            <div>
                <h2 class="text-[10px] font-bold text-[#8c4335]/70 uppercase tracking-widest">Selamat Datang</h2>
                <h1 class="text-lg md:text-2xl font-bold text-slate-800 tracking-tight mt-0.5"><?php echo htmlspecialchars($userName); ?></h1>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <span class="hidden md:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-800 border border-emerald-500/10">
                    <span class="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Kas Aktif
                </span>
            </div>
        </header>
        ...
        <main class="flex-1 p-4 md:p-8 space-y-6 overflow-y-auto max-w-7xl w-full mx-auto">
            ...
            <div class="grid gap-4 grid-cols-1 sm:grid-cols-3">
                <div class="bg-white/50 backdrop-blur-xl border border-white/60 p-6 rounded-[24px] shadow-[0_10px_30px_rgba(140,67,53,0.03)] flex flex-col justify-between">
                    ...
                    <p class="mt-4 text-[11px] text-slate-500 font-medium">Saldo bersih kas kelompok Anda.</p>
                </div>
                <div class="bg-white/50 backdrop-blur-xl border border-white/60 p-6 rounded-[24px] shadow-[0_10px_30px_rgba(140,67,53,0.03)] flex flex-col justify-between">
                    <div>
                    ...
                    <p class="mt-4 text-[11px] text-slate-500 font-medium">Jumlah total dana masuk.</p>
                </div>
                <div class="bg-white/50 backdrop-blur-xl border border-white/60 p-6 rounded-[24px] shadow-[0_10px_30px_rgba(140,67,53,0.03)] flex flex-col justify-between">
                    <div>
                    ...
                    <p class="mt-4 text-[11px] text-slate-500 font-medium">Total biaya kegiatan kelompok.</p>
                </div>
            </div>
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="bg-white/50 backdrop-blur-xl border border-white/60 rounded-[28px] shadow-[0_15px_35px_rgba(140,67,53,0.03)] p-5 md:p-6 lg:col-span-2">
                    <div class="pb-4 border-b border-black/5">
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">6 aktivitas finansial kelompok terakhir</p>
                    </div>
                        ...
                        <table class="w-full text-left text-sm text-slate-600 border-collapse">
                            ...
                            <tbody class="divide-y divide-black/[0.03]">
                                <?php if (count($transactions) === 0): ?>
                                    <tr>
                                        <td colspan="4" class="py-8 text-center text-slate-400 text-xs font-medium">Belum ada aktivitas transaksi keuangan.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($transactions as $trx): ?>
                                        <tr class="hover:bg-white/40 transition-colors">
                                            ...
                                            <td class="py-3.5 px-2 max-w-[130px] md:max-w-[180px] truncate font-medium text-xs text-slate-800"><?php echo htmlspecialchars($trx['description']); ?></td>
                ...
                <div class="space-y-4">
                    <div class="bg-white/50 backdrop-blur-xl border border-white/60 rounded-[24px] p-5 shadow-[0_10px_30px_rgba(140,67,53,0.02)]">
                        ...
                        <p class="text-xs text-slate-500 leading-relaxed font-medium">
                            Pastikan mencatatkan semua bukti nota fisik ke dalam modul <strong>Pemasukan</strong> atau <strong>Pengeluaran</strong> agar rekap kas terverifikasi secara otomatis.
                        </p>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-[#5f312d] to-[#8c4335] text-white rounded-[24px] p-5 shadow-[0_15px_35px_rgba(140,67,53,0.15)] relative overflow-hidden">
                        <h4 class="text-[11px] font-bold uppercase tracking-wider mb-2 text-white/90">Butuh Bantuan?</h4>
                        <p class="text-xs text-white/80 leading-relaxed mb-4 font-light">Jika mengalami kendala hak akses akun atau salah input data, silakan koordinasi dengan Admin Utama Kampus.</p>
                        <a href="profil.php" class="inline-block text-xs font-semibold text-[#5f312d] bg-white hover:bg-white/90 px-4 py-2 rounded-xl shadow-sm transition-all active:scale-95">Lihat Detail Akun</a>
                    </div>
                </div>
